...
const ParallaxElements = () => {
    ...
    useEffect(() => {
        const parallaxElement = parallaxContainerRef.current;

        if (parallaxElement) {
            const handleMouseMove = (event) => {
                const width = parallaxElement.clientWidth;
                const height = parallaxElement.clientHeight;
                ...
                layers.forEach((layer) => {
                    ...
                    const offsetX = (mouseX - centerX) * depth * 0.2;
                    const offsetY = (mouseY - centerY) * depth * 0.2;

                    layer.style.transform = `translate(${offsetX}px, ${offsetY}px)`;
                });
            };

            parallaxElement.addEventListener('mousemove', handleMouseMove);

            // Cleanup function to remove event listener on unmount
            return () => parallaxElement.removeEventListener('mousemove', handleMouseMove);
        }
    }, []);

    return (
        <div ref={parallaxContainerRef} class="parallax" data-relative-input="true">
